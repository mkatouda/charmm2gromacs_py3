...
try:
    ...
    print('Cannot create output directory. Does it already exist?')
os.chdir(ffName)
...
for line in topFile:
    ...
    if not presFlag:
        ...
        if line.startswith('ATOM'):
            ...
            charge = segments[3]
            try:
                type2charge[typei]=charge
            except KeyError:
                None
            if not lipidFlag:
                aartpFile.write('\t'+name+'\t'+typei+'\t'+charge+'\t'+str(groupCounter)+'\n')
            else:
                lipidrtpFile.write('\t'+name+'\t'+typei+'\t'+charge+'\t'+str(groupCounter)+'\n')
            groupCounter += 1
...
#print('type2element:', type2element)
...
for line in parFile:
    ...
    if len(line) > 0:
        ...
        if impropers and not nonbonded and not cmap:
            ...
            # ignore comments and the first IMPROPER line
            if line[0] != comment and not segments[0].startswith('IMPROPER'):
                typei = segments[0]
                typej = segments[1]
                typek = segments[2]
                typel = segments[3]
            ...
            if line[0]!=comment and segments[0]!='NONBONDED' and segments[0]!='cutnb' and segments[0]!='CUTNB':
                ...
                if len(segments)> 6:
                    ...
                    if not isinstance(segments[5],str): # not string?
                        ...
                        # add to list
                        paramList.append([typei, eps14, sigma14])
                # test if partial charge is defined
                try:
                    charge = type2charge[typei]
                    noChargeComment = ''
                except KeyError:
                ...
                if typei == 'HT':
                    ...
                    nbFile.write(typei+'\t'+
                                 element2atomNumber[type2element[typei]]+'\t'+
                                 str(4*float(element2mass[type2element[typei]]))+'\t'+
                                 charge+'\t'+
                                 ptype+'\t'+
                                 sigma+'\t'+
                    ...
                    nbFile.write(typei+'\t'+
                                 element2atomNumber[type2element[typei]]+'\t'+
                                 element2mass[type2element[typei]]+'\t'+
                                 charge+'\t'+
                                 ptype+'\t'+
                                 sigma+'\t'+
                                 eps+' '+noChargeComment+'\n')
                    ...
                    if typei in type2element:
                    #if type2element.has_key(typei):
                        print(typei, " ->", type2element)
                        nbFile.write(typei+'\t'+
                                 element2atomNumber[type2element[typei]]+'\t'+
                                 element2mass[type2element[typei]]+'\t'+
                                 charge+'\t'+
